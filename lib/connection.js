    ...
    try {
      ...
      if(logger) {
        try {
          ...
          logger.customLog({ channel }, 'NewRabbitMqConnnection', { result: 'success' }); 
        } catch (error) {
          debug(error);
        }
      }
      ...
      if(logger) {
        try {
          debug('Logger is enabled, sending feed for NewRabbitMqConnnection...');
          logger.customLog({ channel }, 'ConnnectionError', { error: err.trace || err.message || JSON.stringify(err) }); 
        } catch (error) {
          debug(error);
        }
      }
